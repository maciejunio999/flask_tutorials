/*
  working with the classes People and CreatePersonForm
*/
...
export class People {
    ...
    activateCreateForm() {
      // looking for the form within .person-create-card and setting it to peopleForm 
      const peopleForm = document.querySelector(".person-create-card form");
      // instantiating CreatePersonForm with peopleForm as an argument
...
class CreatePersonForm {
  ...
  // when you click the button, then you make a POST request to the API
  ...
  // when .addPersonToList() executes, you clone the first .person-card as a personCard template and add the new person’s data
  // when you create a new person, you copy the data of an existing person
  addPersonToList(rawData) {
    ...
    // to avoid copying the existing notes along with the person, you delete them
    personCard
      .querySelectorAll(".note-card")
      .forEach((noteCard) => noteCard.remove());
    new PersonControl(personCard);

    // instantiating a new NoteCreateForm, you ensure that the note-creation form connects to the right person
    new NoteCreateForm(personCard.querySelector(".note-list"), data.id);
    // add personCard to your list of people
